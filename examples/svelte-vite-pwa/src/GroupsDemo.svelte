<script lang="ts">
  ...
  export let evolu: EvoluWithGroups;
  
  // Check if groups are supported
  const supportsGroups = hasGroupSupport(evolu);
  
  // Reactive stores
  const currentGroupState = supportsGroups ? currentGroupStore(evolu) : null;
  const groupsState = supportsGroups ? groupsStore(evolu) : null;
  ...
  // Reactive state for selected group
  $: selectedGroupState = selectedGroupId ? groupStore(evolu, () => selectedGroupId) : null;
  $: membersState = selectedGroupId ? groupMembersStore(evolu, () => selectedGroupId) : null;
  ...
  {:else}
    ...
    <section class="current-group-section">
      ...
      {#if currentGroupState?.group}
        <div data-testid="current-group">
          <p>Group ID: <code>{currentGroupState.group.groupId}</code></p>
          <p>Role: <code>{currentGroupState.group.role}</code></p>
    ...
    <section class="groups-list-section">
      ...
      {:else if groupsState?.error}
        <p class="error" data-testid="groups-error">Error loading groups</p>
      {:else if !groupsState?.groups || groupsState.groups.length === 0}
          ...
          {#each groupsState.groups as group}
            <li class="group-item">
              ...
              <button 
                ...
                class:active={currentGroupState?.group?.groupId === group.id}
                data-testid="set-context-{group.id}"
              >
                {currentGroupState?.group?.groupId === group.id ? 'Current' : 'Set as Current'}
              </button>
            </li>
          {/each}
        </ul>
    ...
    {#if selectedGroupId && selectedGroupState}
      <div class="group-details-overlay">
        <div class="group-details" data-testid="group-details">
          <h3>Group: {selectedGroupState.group?.name || 'Loading...'}</h3>
          <button on:click={closeDetails}>Close</button>
          
          {#if selectedGroupState.loading}
            <p data-testid="group-details-loading">Loading group details...</p>
          {:else if selectedGroupState.error}
            <p class="error" data-testid="group-details-error">Error loading group</p>
          {:else if !selectedGroupState.group}
            <p data-testid="group-not-found">Group not found</p>
          {:else}
            <div class="group-info">
              <p>ID: <code>{selectedGroupState.group.id}</code></p>
              <p>Created: {new Date(selectedGroupState.group.createdAt).toLocaleDateString()}</p>
              <p>Current Epoch: {selectedGroupState.group.currentEpoch}</p>
            </div>

            <h4>Members ({membersState?.members.length || 0})</h4>
            {#if membersState?.loading}
              <p>Loading members...</p>
            {:else if membersState?.error}
              <p class="error">Error loading members</p>
            {:else}
