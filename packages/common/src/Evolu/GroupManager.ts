...
import { SimpleEpochManager } from "./EpochManager.js";
...
export const createGroupManager = (
  ...
  const createGroupWithMembers = (
    ...
    const epochManager = new SimpleEpochManager(
      group.currentEpoch,
      []
    );
    const authProvider = createGroupAuthProvider(
      group.id,
      deps.currentUserId,
      activeMembers.map(m => ({
        userId: m.userId,
    ...
    const securityContext = createGroupSecurityContext(group, epochManager);
